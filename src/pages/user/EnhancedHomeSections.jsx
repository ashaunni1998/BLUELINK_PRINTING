...
const EnhancedHomeSections = () => {
  ...
  id="flyers-section"
  data-animate
  className={`relative py-12 sm:py-16 md:py-20 lg:py-32 overflow-hidden transition-all duration-1000 delay-200 bg-blue-50 mx-[3%] sm:mx-[4%] md:mx-[5%] lg:mx-[6%] ${
    visibleSections['flyers-section']
      ? 'opacity-100 translate-y-0'
      : 'opacity-0 translate-y-10'
  }`}
>
  {/* Responsive container with desktop-specific styling */}
  <div className="flyers-container mx-auto px-4 sm:px-6 md:px-8 lg:px-4">
    <style jsx>{`
    ...
    <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 md:gap-12 lg:gap-16 items-center">
      ...
      <div className="relative group order-2 lg:order-1">
        <div className="relative bg-white p-3 sm:p-4 md:p-5 lg:p-6 rounded-2xl sm:rounded-3xl border-2 border-blue-200 max-w-sm sm:max-w-md md:max-w-lg mx-auto lg:mx-0">
          <img
            ...
            alt="Flyers & Leaflets"
            className="w-full rounded-xl sm:rounded-2xl transform group-hover:scale-105 transition-transform duration-500"
          />
          
          {/* Floating Elements - Responsive */}
          <div className="absolute -top-3 -right-3 sm:-top-4 sm:-right-4 bg-white rounded-xl sm:rounded-2xl p-2 sm:p-3 lg:p-4 border-2 border-blue-200 shadow-lg">
            <div className="text-center">
              <div className="text-sm sm:text-base md:text-lg lg:text-xl font-black text-blue-600">Fast</div>
              <div className="text-[10px] sm:text-xs text-gray-600">Delivery</div>
            </div>
          </div>
          
          <div className="absolute -bottom-3 -left-3 sm:-bottom-4 sm:-left-4 bg-blue-600 text-white rounded-xl sm:rounded-2xl p-2 sm:p-3 lg:p-4 shadow-lg">
            <div className="text-center">
              <div className="text-sm sm:text-base md:text-lg lg:text-xl font-black">HD</div>
              <div className="text-[10px] sm:text-xs">Quality</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 sm:space-y-6 md:space-y-7 lg:space-y-8 order-1 lg:order-2">
        <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-100 border border-blue-200 rounded-full text-blue-700 text-xs sm:text-sm font-medium">
          <FileText className="w-3 h-3 sm:w-4 sm:h-4" />
          Marketing Materials
        </div>

        <div className="space-y-4 sm:space-y-5 md:space-y-6">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl font-black text-gray-900 leading-tight">
            Flyers & Leaflets.
            <span className="block text-blue-600">Spread the word.</span>
          </h2>
          <div className="w-20 sm:w-24 h-1 bg-blue-600 rounded-full" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          {[
            ...
            <div key={index} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 md:p-3 bg-blue-100 rounded-lg sm:rounded-xl border border-blue-200">
              <feature.icon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 flex-shrink-0" />
              <span className="text-xs sm:text-sm font-medium text-gray-700 leading-tight">{feature.text}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <button className="group flex items-center justify-center gap-2 px-4 sm:px-5 md:px-6 lg:px-8 py-3 sm:py-3.5 md:py-4 bg-blue-600 text-white text-sm sm:text-base font-bold rounded-xl sm:rounded-2xl transform transition-all duration-300 hover:scale-105 w-full sm:w-auto">
            <span className="whitespace-nowrap">Shop Flyers & Leaflets</span>
            <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform flex-shrink-0" />
          </button>
          <button className="px-4 sm:px-5 md:px-6 lg:px-8 py-3 sm:py-3.5 md:py-4 border-2 border-blue-400 text-blue-600 text-sm sm:text-base font-semibold rounded-xl sm:rounded-2xl bg-white hover:bg-blue-50 transition-all duration-300 w-full sm:w-auto">
            View Samples
          </button>
        </div>
      </div>
